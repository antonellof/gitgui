...
impl Mods {
    pub const NONE: Mods = Mods { shift: false, alt: false, ctrl: false, sup: false };
    #[cfg(test)]
    pub const SHIFT: Mods = Mods { shift: true, alt: false, ctrl: false, sup: false };
    pub const ALT: Mods = Mods { shift: false, alt: true, ctrl: false, sup: false };
    pub const CTRL: Mods = Mods { shift: false, alt: false, ctrl: true, sup: false };
...
impl Parser {
    pub fn new(pixel_mouse: bool, cell_w: u32, cell_h: u32) -> Self {
        Self { buf: Vec::with_capacity(256), pixel_mouse, cell_w: cell_w.max(1), cell_h: cell_h.max(1) }
    }
        ...
        loop {
            ...
            match self.step() {
                ...
                Step::Incomplete => {
                    ...
                    } else if self.buf[0] == 0x1b && self.buf.len() >= 2 && self.buf[1] != b'[' && self.buf[1] != b'O' {
    ...
    fn step(&self) -> Step {
        ...
        match b[1] {
            ...
            b'O' => {
                ...
                let Some(&f) = b.get(2) else { return Step::Incomplete };
                let key = match f {
                    b'A' => Some(Key::Up),
                    b'B' => Some(Key::Down),
                    b'C' => Some(Key::Right),
            ...
            b'_' | b']' | b'P' | b'^' => {
                ...
                let bel = if b[1] == b']' { body.iter().position(|&c| c == 0x07).map(|p| p + 1) } else { None };
                match (st, bel) {
            ...
            other => {
                ...
                if other >= 0x80 {
                    ...
                    if let Ok(s) = std::str::from_utf8(&b[1..1 + need]) {
                        ...
                        return Step::Done(1 + need, Some(key_event(Key::Char(c), Mods::ALT, None)));
                    }
    ...
    fn step_plain(&self) -> Step {
        ...
        match std::str::from_utf8(&b[..need]) {
            Ok(s) => {
                ...
                Step::Done(need, Some(key_event(Key::Char(ch), Mods::NONE, Some(ch.to_string()))))
            }
    ...
    fn step_csi(&self) -> Step {
        ...
        let ev = match fin {
            ...
            b'M' | b'm' if params.first() == Some(&b'<') => self.parse_sgr_mouse(&params[1..], fin == b'M'),
            ...
            b'~' => {
                ...
                let mods = it.next().and_then(parse_num).map(Mods::from_param).unwrap_or(Mods::NONE);
                let key = match num {
                    2 => Some(Key::Insert),
                    3 => Some(Key::Delete),
                    5 => Some(Key::PageUp),
    ...
    fn parse_sgr_mouse(&self, params: &[u8], is_press_or_motion: bool) -> Option<Event> {
        ...
        Some(Event::MouseButton { button, pressed: is_press_or_motion, x, y, mods })
    }
}

fn key_event(key: Key, mods: Mods, text: Option<String>) -> Event {
    Event::Key { key, mods, text, pressed: true, repeat: false }
}
...
fn legacy_mods(params: &[u8]) -> Mods {
    ...
    match it.next() {
        ...
        None if !first.is_empty() && first != b"1" => parse_num(first).map(Mods::from_param).unwrap_or(Mods::NONE),
        None => Mods::NONE,
    }
}
...
fn parse_kitty_key(params: &[u8]) -> Option<Event> {
    ...
    Some(Event::Key { key, mods, text, pressed, repeat })
}
...
mod tests {
    ...
    fn kitty_plain_letter_has_text() {
        assert_eq!(feed(b"\x1b[113u"), vec![key(Key::Char('q'), Mods::NONE, Some("q"))]);
        assert_eq!(feed(b"\x1b[113;1u"), vec![key(Key::Char('q'), Mods::NONE, Some("q"))]);
        assert_eq!(feed(b"\x1b[113;1:1u"), vec![key(Key::Char('q'), Mods::NONE, Some("q"))]);
    }
    ...
    fn kitty_repeat_and_release() {
        assert_eq!(
            feed(b"\x1b[106;1:2u"),
            vec![Event::Key { key: Key::Char('j'), mods: Mods::NONE, text: Some("j".into()), pressed: true, repeat: true }]
        );
        assert_eq!(
            feed(b"\x1b[106;1:3u"),
            vec![Event::Key { key: Key::Char('j'), mods: Mods::NONE, text: None, pressed: false, repeat: false }]
        );
    }
    ...
    fn kitty_shift_uses_shifted_key_or_uppercase() {
        ...
        assert_eq!(feed(b"\x1b[97:65;2u"), vec![key(Key::Char('a'), Mods::SHIFT, Some("A"))]);
        // shift+1 -> '!'
        assert_eq!(feed(b"\x1b[49:33;2u"), vec![key(Key::Char('1'), Mods::SHIFT, Some("!"))]);
        // no alternate key: uppercase ascii
        assert_eq!(feed(b"\x1b[112;2u"), vec![key(Key::Char('p'), Mods::SHIFT, Some("P"))]);
    }

    #[test]
    fn kitty_ctrl_and_alt_have_no_text() {
        assert_eq!(feed(b"\x1b[99;5u"), vec![key(Key::Char('c'), Mods::CTRL, None)]);
        assert_eq!(feed(b"\x1b[120;3u"), vec![key(Key::Char('x'), Mods::ALT, None)]);
        // ctrl+shift+p: modifier 6
        assert_eq!(
            feed(b"\x1b[112;6u"),
            vec![key(Key::Char('p'), Mods { shift: true, ctrl: true, ..Mods::NONE }, None)]
        );
    }
    ...
    fn kitty_text_field_wins() {
        ...
        assert_eq!(feed(b"\x1b[97;1;228u"), vec![key(Key::Char('a'), Mods::NONE, Some("ä"))]);
    }

    #[test]
    fn kitty_lock_modifiers_are_masked() {
        // caps lock (64) + shift (1): param 66
        assert_eq!(feed(b"\x1b[97:65;66u"), vec![key(Key::Char('a'), Mods::SHIFT, Some("A"))]);
        // num lock alone: param 129 -> no mods
        assert_eq!(feed(b"\x1b[57399;129u"), vec![key(Key::Char('0'), Mods::NONE, Some("0"))]);
    }
    ...
    fn kitty_functional_keys() {
        ...
        assert_eq!(feed(b"\x1b[127u"), vec![key(Key::Backspace, Mods::NONE, None)]);
        assert_eq!(feed(b"\x1b[57348u"), vec![key(Key::Insert, Mods::NONE, None)]);
        assert_eq!(feed(b"\x1b[57349u"), vec![key(Key::Delete, Mods::NONE, None)]);
        assert_eq!(feed(b"\x1b[57350u"), vec![key(Key::Left, Mods::NONE, None)]);
        assert_eq!(feed(b"\x1b[57351u"), vec![key(Key::Right, Mods::NONE, None)]);
        assert_eq!(feed(b"\x1b[57352u"), vec![key(Key::Up, Mods::NONE, None)]);
        assert_eq!(feed(b"\x1b[57353u"), vec![key(Key::Down, Mods::NONE, None)]);
        assert_eq!(feed(b"\x1b[57354u"), vec![key(Key::PageUp, Mods::NONE, None)]);
        assert_eq!(feed(b"\x1b[57355u"), vec![key(Key::PageDown, Mods::NONE, None)]);
        ...
        assert_eq!(feed(b"\x1b[57375u"), vec![key(Key::F(12), Mods::NONE, None)]);
        // space is text
        assert_eq!(feed(b"\x1b[32u"), vec![key(Key::Char(' '), Mods::NONE, Some(" "))]);
        // left shift press: Other, no text
        assert_eq!(feed(b"\x1b[57441;2u"), vec![key(Key::Other(57441), Mods::SHIFT, None)]);
    }
    ...
    fn fallback_text_ctrl_and_alt() {
        ...
        assert_eq!(feed("é".as_bytes()), vec![key(Key::Char('é'), Mods::NONE, Some("é"))]);
        ...
        assert_eq!(feed(b"ab"), vec![key(Key::Char('a'), Mods::NONE, Some("a")), key(Key::Char('b'), Mods::NONE, Some("b"))]);
    }
    ...
    fn utf8_split_across_feeds() {
        ...
        assert_eq!(p.feed(&bytes[1..]), vec![key(Key::Char('é'), Mods::NONE, Some("é"))]);
        assert!(!p.has_pending());
    }
    ...
    fn sgr_mouse_pixels() {
        assert_eq!(
            ...
            vec![Event::MouseButton { button: MouseButton::Left, pressed: true, x: 100, y: 200, mods: Mods::NONE }]
        );
        assert_eq!(
            feed(b"\x1b[<0;101;201m"),
            vec![Event::MouseButton { button: MouseButton::Left, pressed: false, x: 100, y: 200, mods: Mods::NONE }]
        );
        assert_eq!(
            feed(b"\x1b[<2;1;1M"),
            vec![Event::MouseButton { button: MouseButton::Right, pressed: true, x: 0, y: 0, mods: Mods::NONE }]
        );
        assert_eq!(
            feed(b"\x1b[<1;5;5M"),
            vec![Event::MouseButton { button: MouseButton::Middle, pressed: true, x: 4, y: 4, mods: Mods::NONE }]
        );
        // motion with no button: 32 + 3
        assert_eq!(feed(b"\x1b[<35;11;21M"), vec![Event::MouseMove { x: 10, y: 20, mods: Mods::NONE }]);
        // drag with left button: 32 + 0
        assert_eq!(feed(b"\x1b[<32;11;21M"), vec![Event::MouseMove { x: 10, y: 20, mods: Mods::NONE }]);
        // shift+click: 4
        assert_eq!(
            feed(b"\x1b[<4;11;21M"),
            vec![Event::MouseButton { button: MouseButton::Left, pressed: true, x: 10, y: 20, mods: Mods::SHIFT }]
        );
        // ctrl+drag: 16 + 32
        assert_eq!(feed(b"\x1b[<48;11;21M"), vec![Event::MouseMove { x: 10, y: 20, mods: Mods::CTRL }]);
    }

    #[test]
    fn sgr_wheel() {
        assert_eq!(feed(b"\x1b[<64;11;21M"), vec![Event::Wheel { dx: 0, dy: 1, x: 10, y: 20, mods: Mods::NONE }]);
        assert_eq!(feed(b"\x1b[<65;11;21M"), vec![Event::Wheel { dx: 0, dy: -1, x: 10, y: 20, mods: Mods::NONE }]);
        assert_eq!(feed(b"\x1b[<66;11;21M"), vec![Event::Wheel { dx: 1, dy: 0, x: 10, y: 20, mods: Mods::NONE }]);
        assert_eq!(feed(b"\x1b[<67;11;21M"), vec![Event::Wheel { dx: -1, dy: 0, x: 10, y: 20, mods: Mods::NONE }]);
        // shift+wheel down: 65 + 4
        assert_eq!(feed(b"\x1b[<69;11;21M"), vec![Event::Wheel { dx: 0, dy: -1, x: 10, y: 20, mods: Mods::SHIFT }]);
    }
    ...
    fn sgr_mouse_cells_convert_to_pixel_centers() {
        ...
        assert_eq!(
            p.feed(b"\x1b[<0;3;2M"),
            vec![Event::MouseButton { button: MouseButton::Left, pressed: true, x: 2 * 16 + 8, y: 34 + 17, mods: Mods::NONE }]
        );
    }
    ...
    fn focus_and_paste() {
        ...
        assert_eq!(feed(b"\x1b[200~hello\nworld\x1b[201~"), vec![Event::Paste("hello\nworld".into())]);
        // Paste split across feeds
        let mut p = Parser::new(true, 1, 1);
        assert!(p.feed(b"\x1b[200~par").is_empty());
        assert_eq!(p.feed(b"tial\x1b[201~x"), vec![Event::Paste("partial".into()), key(Key::Char('x'), Mods::NONE, Some("x"))]);
    }
    ...
    fn incomplete_then_complete_and_flush() {
        ...
        assert_eq!(
            p.feed(b";20M"),
            vec![Event::MouseButton { button: MouseButton::Left, pressed: true, x: 9, y: 19, mods: Mods::NONE }]
        );
        ...
        assert_eq!(p.feed(b"\x1b\x1b"), vec![key(Key::Escape, Mods::NONE, None)]);
        assert_eq!(p.flush(), vec![key(Key::Escape, Mods::NONE, None)]);
    }
    ...
    fn mixed_stream_in_one_feed() {
        ...
        assert_eq!(evs.len(), 4);
        assert!(matches!(evs[0], Event::Key { key: Key::Char('q'), .. }));
        assert!(matches!(evs[1], Event::MouseMove { .. }));
        assert!(matches!(evs[2], Event::Key { key: Key::Char('q'), .. }));
    ...
    fn terminal_replies_are_skipped() {
        ...
        assert_eq!(feed(b"\x1b[?62;22c"), vec![Event::Unknown(b"\x1b[?62;22c".to_vec())]);

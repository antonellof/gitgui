...
- Editor layout: while a file is open the main area switches to three columns: sidebar, a column with the commit list above the file lists (working tree lists and commit box, or the commit's files), and the editor at full height. Commit rows narrower than 420 pt drop the author column. Closing the editor restores the two-row layout.
